...
template <typename T> struct RoverT {
    ...
    void update(const std::array<T, N_THRUSTERS> &thrusts) {
        ...
        for (size_t i = 0; i < N_THRUSTERS; i++) {
            net_thrust_torque +=
                thrust_positions[i].cross(thrust_vectors[i] * thrusts[i]);
        }

        // Get net buoyancy
        Vector3<T> buoyancy_force =
            accel_gravity * water_density * volume *
            Vector3<T>{T{0}, T{0}, T{1}}; // Inertial frame
        ...
        Vector3<T> net_force_inertial =
            buoyancy_force + attitude.value * net_thrust_force;

        // Compute equations of motion
        position.derivative = velocity.value;
        velocity.derivative = net_force_inertial / (mass + m_added);
        angvel.derivative = moi_inv * (net_torque_body -
                                       angvel.value.cross(moi * angvel.value));
