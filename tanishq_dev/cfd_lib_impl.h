...
void CFDMesh<T, MeshParams>::ppe_laplacian_product(
    ...
    run_kernel([&source, &destination](MeshIndex xi, MeshIndex yi,
                                       MeshIndex zi) {
        const auto p_imjk = xi == 0 ? 0.0 : source(xi - 1, yi, zi);
        const auto p_ipjk =
            xi == MeshParams::nX - 1 ? 0.0 : source(xi + 1, yi, zi);
        const auto p_ijmk = yi == 0 ? 0.0 : source(xi, yi - 1, zi);
        const auto p_ijpk =
            yi == MeshParams::nY - 1 ? 0.0 : source(xi, yi + 1, zi);
        const auto p_ijkm = zi == 0 ? 0.0 : source(xi, yi, zi - 1);
        const auto p_ijkp =
            zi == MeshParams::nZ - 1 ? 0.0 : source(xi, yi, zi + 1);
        const auto p_ijk = source(xi, yi, zi);

        const auto div_x =
            (p_imjk - 2 * p_ijk + p_ipjk) / (MeshParams::dx * MeshParams::dx);
        const auto div_y =
            (p_ijmk - 2 * p_ijk + p_ijpk) / (MeshParams::dy * MeshParams::dy);
        const auto div_z =
            (p_ijkm - 2 * p_ijk + p_ijkp) / (MeshParams::dz * MeshParams::dz);
...
void CFDMesh<T, MeshParams>::compute_pressure() {
    ...
    for (size_t i = 0; i < MeshParams::ppe_max_iterations; i++) {
        const bool converged =
            bicgstab_step(MeshParams::ppe_s_stop, MeshParams::ppe_x_stop);
        if (converged) {
            break;
        }
    }
...
void CFDMesh<T, MeshParams>::compute_next_velocity() {
    ...
    run_kernel([&boundary, &P, &uStar, &vStar, &wStar, &uNext, &vNext,
        ...
        uNext(xi, yi, zi) = uStar(xi, yi, zi) -
                            (MeshParams::dt / rho) * P.partial_x(xi, yi, zi);
        vNext(xi, yi, zi) = vStar(xi, yi, zi) -
                            (MeshParams::dt / rho) * P.partial_y(xi, yi, zi);
        wNext(xi, yi, zi) = wStar(xi, yi, zi) -
                            (MeshParams::dt / rho) * P.partial_z(xi, yi, zi);
    });
}
    ...
    for (size_t xi = 0; xi < MeshParams::nX; xi++) {
        for (size_t yi = 0; yi < MeshParams::nY; yi++) {
            for (size_t zi = 0; zi < MeshParams::nZ; zi++) {
                const auto dF =
                    normals(xi, yi, zi) * this->pressure(xi, yi, zi);
                force += dF;
